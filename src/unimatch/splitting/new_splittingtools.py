...
import unimatch.decoders.unifiedDecodertools as udt 
...
def generate_splitting_data_unified(L, p_list, P_0, err_0, c_max, nsamples, weight_A):
    """
    Returns an analytical estimate array, and a numerical estimate from the splitting method, estimated using nsamples Metropolis samples.
    L : size of the lattice
    """
    ...
    m = Matching(H_graph)
    weights = [udt.test_weights_config_c(my_code, udt.get_fault_ids(H_unified, (e[0],e[1])), [weight_A, 1]) for e in m.edges()]
